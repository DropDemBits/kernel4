...
static const struct dnode_ops ttyfs_dops =
{
...
static const struct inode_ops ttyfs_iops = 
{
    .open = default_open,
    .close = default_close,
    .read = ttyfs_read,
...
struct fs_instance* ttyfs_create()
{
    ...
    construct_dnode(instance, root_dir, root_inode, "/");
    construct_inode(instance, root_inode);
...
void ttyfs_add_tty(struct ttyfs_instance* instance, tty_dev_t* dev, const char* path)
{
    ...
    construct_dnode((struct fs_instance*)instance, (struct dnode*)dnode, inode, path);
