...
#define FIXUP_BAR(x, val) \
    if(!(x & 0xFE)) { klog_logln(LVL_DEBUG, "PATA: Fixing up IDE quirk (%#lx -> %#lx)", x, val); (x) = (val); }
...
static irq_ret_t pata_irq_handler(struct irq_handler* handler)
{
    ...
    {
        klog_logln(LVL_WARN, "IRQ fired outside of command");
    }
...
static bool ata_wait(bool only_irq)
{
    ...
    while(!irq_fired)
    {
        ...
        {
            klog_logln(LVL_DEBUG, "ata_dev%d timeout %x (nirqs: %d, %d)", current_id, inb(current_device->control_base + ATA_ALT_STATUS), num_irqs, timeout - timer_read_counter(0));
            return true;
        }
    }

    klog_logln(LVL_DEBUG, "ata_dev%d nt %x", current_id, inb(current_device->control_base + ATA_ALT_STATUS));
...
int pata_do_transfer(uint16_t id, uint64_t lba, void* transfer_buffer, uint32_t sector_count, int transfer_dir, bool is_dma, bool is_48bit)
{
    ...
    klog_logln(LVL_DEBUG, "(ata_dev%d) Beginning PIO Data Transfer (%lld bytes)", id, (uint64_t)(word_count << 1));
...
int atapi_send_command(uint16_t id, uint16_t* command, void* transfer_buffer, uint16_t transfer_size, int transfer_dir, bool is_dma, bool is_16b)
{
    ...
    do
    {
        ...
        klog_logln(LVL_DEBUG, "(ata_dev%d) Begining Packet PIO Data Transfer (%d bytes)", id, byte_count);
...
struct pata_dev* init_ide_controller(uint32_t command_base, uint32_t control_base, uint16_t irq_line, bool secondary)
{
    ...
    if(err == EABSENT)
    {
        klog_logln(LVL_DEBUG, "ata_dev%d is absent, disabling", controller->dev.id);
    ...
    else if(id_low == 0x3C && id_hi == 0xC3) klog_logln(LVL_DEBUG, "Detected SATA Enclosure Management Bridge");
    else if(id_low == 0x69 && id_hi == 0x96) klog_logln(LVL_DEBUG, "Detected SATA Port Multiplier");
    else if(id_low == 0x00 && id_hi == 0x00) controller->dev.device_type = TYPE_ATA;
    else
    {
        klog_logln(LVL_ERROR, "ata_dev%d: Unknown Device Sig: %x %x", controller->dev.id, id_low, id_hi);
        controller->dev.is_active = false;
        controller->dev.processing_command = false;
        current_id = ATA_INVALID_ID;
        current_device = KNULL;
    ...
    klog_logln(LVL_DEBUG, "ATA Device Present (%d, %s)", controller->dev.id, type_names[controller->dev.device_type]);
...
pci_handle_ret_t ata_init_controller(struct pci_dev* dev)
{
    ...
    klog_logln(LVL_INFO, "Initializing IDE controller at %x:%x.%x", dev->bus, dev->device, dev->function);
    ...
    if((pci_read(dev, PCI_PROG_IF, 1) & 0b1010) != 0)
    {
        ...
        klog_logln(LVL_DEBUG, "NewOpMode: %x", pci_read(dev, PCI_PROG_IF, 1));
    }
    ...
    {
        klog_logln(LVL_ERROR, "Operating mode mismatch (%d != %d)", (operating_mode & 0b0001), (operating_mode & 0b0100));
    ...
    {
        klog_logln(LVL_DEBUG, "Fixing up secondary channel");
    ...
    klog_logln(LVL_DEBUG, "Primary channel at %#lx-%#lx, %#lx", command0_base, command0_base+7, control0_base);
    klog_logln(LVL_DEBUG, "Secondary channel at %#lx-%#lx, %#lx", command1_base, command1_base+7, control1_base);
    klog_logln(LVL_DEBUG, "OpMode: %x", operating_mode);
    ...
    {
        klog_logln(LVL_ERROR, "No IDE channels exist");
        return PCI_DEV_NOT_HANDLED;
    }

    if(init_primary)
    {
        klog_logln(LVL_INFO, "Setting up primary IDE channel");
    ...
    if(init_secondary)
    {
        klog_logln(LVL_INFO, "Setting up secondary IDE channel");
        // Use legacy interrupt if channel is in compatibility mode
